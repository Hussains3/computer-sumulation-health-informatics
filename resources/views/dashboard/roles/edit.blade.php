...
<div class="bg-white">
    ...
    <div class="flex justify-between px-4 py-2.5 border-b-2 border-gray">
        <h2 class="font-bold">Edit {{$role->name}}</h2>
        @can('role-create')
        ...
        <form method="POST" action="{{ route('roles.update', $role->id) }}">
            ...
            <ul class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 mb-4">
                ...
                <li class="flex items-center">
                    <input class="mr-2" type="checkbox" name="permission[{{ $permission->name }}]" value="{{ $permission->name }}" class='permission' @if (in_array($permission->name, $rolePermissions))
                    checked
                    @endif>
